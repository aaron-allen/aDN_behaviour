---
title: "**Male aDN>TNT courtship behaviour in the dark**"
...
```{r}
genotypes_a <- read_csv(file = "aDN_A_male_dark.csv", col_names = c("video","fly_id","empty","copulation_frame")) %>% select(-empty) %>% mutate(genotype = "a")
genotypes_b <- read_csv(file = "aDN_B_male_dark.csv", col_names = c("video","fly_id","empty","copulation_frame")) %>% select(-empty) %>% mutate(genotype = "b")
genotypes_c <- read_csv(file = "aDN_C_male_dark.csv", col_names = c("video","fly_id","empty","copulation_frame")) %>% select(-empty) %>% mutate(genotype = "c")
genotypes_d <- read_csv(file = "aDN_D_male_dark.csv", col_names = c("video","fly_id","empty","copulation_frame")) %>% select(-empty) %>% mutate(genotype = "d")
genotypes <- bind_rows(genotypes_a,genotypes_b,genotypes_c,genotypes_d)
genotypes$video <- str_to_lower(genotypes$video)
genotypes$video <- str_trim(genotypes$video, side = "both")
head(genotypes)
```


Next we're replacing the "None" values for the flies that didn't copulate within an hour with `NA`. We're also creating 2 new variables - `status` which is used for the cumulative copulation analysis, and `copulation_time` which is just converting the frame number into seconds.


```{r}
genotypes$copulation_frame[genotypes$copulation_frame=="None"] <- NA
genotypes <- genotypes %>% 
    mutate(status = if_else(is.na(copulation_frame),0,1)) %>% 
    mutate(copulation_time = round(as.numeric(genotypes$copulation_frame)/25))
head(genotypes)
```
...
```{r}
rawdata_list <- list.files("./good_data/",recursive = TRUE) %>% str_subset("_ALLDATA.csv")# %>% str_subset("_Male_")
all_rawdata <- tibble()
for (rawdata_file in rawdata_list) {
  temp <- fread(input = paste0("./good_data/",rawdata_file), showProgress = FALSE)
  all_rawdata <- bind_rows(all_rawdata,temp)
}
all_rawdata$FileName <- str_to_lower(all_rawdata$FileName)
head(all_rawdata)
```
...
for (fly in flies) {
  cop_time <- all_genotypes$copulation_frame[all_genotypes$uni_fly==fly] %>% replace_na(22500)
  temp0 <- all_rawdata %>%
    filter(uni_fly == fly) %>% 
    slice(1:cop_time)
...
First "port of call" is of course Courtship Index. And there isn't much of a visual difference. The CI's are lower than what we saw for courtship behaviour in the light, and that is to be expected.

(the stats will follow, but first lets plot the data)

And just a reminder of the genotypes:

a = _Otd-FLP/+; dsx[Gal4.DBD]/+_ (n=21) 

b = _VGlut[dVP16.AD], UAS>stop>TNT/+_ (n=43)

c = _VGlut[dVP16.AD], UAS>stop>TNT/+; dsx[Gal4.DBD]/+_ (n=35)

d = _VGlut[dVP16.AD], UAS>stop>TNT/Otd-FLP; dsx[Gal4.DBD]/+_ (n=29)
...
Looking at the position of the female while the male has it's wing extended, also looks quite similar. Which is in contrast to the "light" experiments. The males still generally keep the female close and in front, but there are more instances of the female being further away and behind. (_NB_: contralateral = left, ipsilateral = right, and the males postion is the centre)
...
When we quantify the mean facing angle, mean distance to other, and minimum distance to other there is no difference between the flies. Bilateral wing index is trending up in the experimental, but we'll have to wait for the stats to see what's what.
...
There is no apparent difference in wing choice index ether (not that there was one in light though). But all indices are lower than what we saw in the light. Again this is to be expected.
